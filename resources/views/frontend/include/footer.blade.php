...
--------------------------->
@if(empty($mobile))
<footer class="{{($authRole=='2')?'footer-artist':''}}">
    <div class="container-fluid">
        @if(!Auth::check() || $authRole=='3')
        <div class="footer-wrapper">
            @foreach ($frontendFooter as $key => $item)
            <div class="footer-block">
                <p class="s2">{{ $item['footerDetails']['footerName'] }}</p>
                <ul>
                    @foreach ($item['footerMenuData'] as $key1 => $item1)
                    <li><a href="{{ $item1 }}" class="">{{ $key1 }}</a></li>
                    @endforeach
                </ul>
            </div>
            @endforeach
            ...
            <div class="footer-block follow-block">
                ...
                <div class="social-icon">
                    <a target="_blank" href="{{ app('App\Models\GlobalSettings')::getSingleSettingVal('fb_link') }}"><img src="{{ asset('public/assets/frontend/img/fb.svg') }}"></a>
                    {{-- <a target="_blank" href="{{ app('App\Models\GlobalSettings')::getSingleSettingVal('twitter_link') }}"><img src="{{ asset('public/assets/frontend/img/tw.svg') }}"></a> --}}
                    <a target="_blank" href="{{ app('App\Models\GlobalSettings')::getSingleSettingVal('insta_link') }}"><img src="{{ asset('public/assets/frontend/img/insta.svg') }}"></a>
                    {{-- <a target="_blank" href="{{ app('App\Models\GlobalSettings')::getSingleSettingVal('twitter_link') }}"><img src="{{ asset('public/assets/frontend/img/yt.svg') }}"></a> --}}
                </div>
                <p class="s2">Also Available On</p>
                <div class="app-store">
                    <a target="_blank" href="{{ app('App\Models\GlobalSettings')::getSingleSettingVal('andriod_link') }}"><img src="{{ asset('public/assets/frontend/img/app_store_icon.svg') }}"></a>
                    <a target="_blank" href="{{ app('App\Models\GlobalSettings')::getSingleSettingVal('ios_link') }}"><img src="{{ asset('public/assets/frontend/img/play_store_icon.svg') }}"></a>
                </div>
            </div>
        </div>
        @endif
        <div class="copy-right">
            <p class="caption">Copyright © {{date('Y')}} fanclub Ltd.</p>
            ...
            <div class="modal-body">
                <div class="sign-in-wrapper">
                    <div class="sign-in-side">
                        <img src="{{ asset('public/assets/frontend/img/close.svg') }}" alt="" class="modal-close close-black" data-bs-dismiss="modal" />
                        <div class="with-password">
                            <h4>Sign In</h4>
                            <p>Already have an account? Welcome back!</p>
                            <form id="loginFormFromPopup" method="POST" action="{{ url('/login-from-popup') }}">
                                @csrf
                                <div>
                                ...
                                <div class="create-link">
                                    <span>New to fanclub?<a href="{{ route('showSignup') }}"> Create Account</a></span>
                                </div>
                            </form>
                        </div>
                    </div>
                    <div class="signin-img-side">
                        <img src="{{ asset('public/assets/frontend/img/white-close.png') }}" class="white-close" alt="" data-bs-dismiss="modal" />
                        <h6>An exciting exclusive music collection to complement your lifestyle. Created by music fans for music fans</h6>
                        <div class="img-wrapper">
                            <img src="{{ asset('public/assets/frontend/img/modal-img.png') }}" alt="" />
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="sign-in-wrapper">
                    <div class="sign-in-side">
                        ...
                        <div class="with-password add-padding-otp-email">
                            <h4>Sign In</h4>
                            <p>Already have an account? Welcome back!</p>
                            <form id="loginWithOtpFormFromPopup" method="POST" action="{{ url('/login-using-otp-from-popup') }}">
                                @csrf
                                <div>
                                    <div class="inputs-group">
                    ...
                    <div class="sign-in-side">
                        ...
                        <div class="with-password otp-page add-padding-otp">
                            ...
                            <div class="email-edit">
                                ...
                                <a data-bs-target="#loginWithOtpModal" data-bs-toggle="modal" data-bs-dismiss="modal"><img src="{{ url('public/assets/img/edit.svg') }}" alt=""></a>
                            </div>
                            <form id="loginWithOtpVerificationFormFromPopup" method="POST" action="{{ url('/otp-verification-from-popup') }}">
                                @csrf
                                <div class="otp-input">
                            ...
                            <div class="resend-otp">
                                <form action="{{ url('/login-using-otp-from-popup') }}" id="resendOtpFormFromPopup" method="post">
                                    <input type="hidden" name="_token" id="token" value="{{ csrf_token() }}">
                                    <input type="hidden" name="input" class="opt-email-popup-value" id="email" value="{{ Session::get('opt-email') }}" />
                                    <span>
                                        <button class="resend-button">Resend OTP</button>
                                        {{-- <a href="javascript:void(0)" onclick="document.getElementById('resendOtpFormFromPopup').submit();">Resend OTP</a> --}}
                                    </span>
                                </form>
                            </div>
                        </div>
                        ...
                        <input type="hidden" name="_token" id="token" value="p5UNrpLfORJpv0Rs4Xa9jY310NhvcaK3tmhQmHfU">
                        <input type="hidden" name="input" id="email" value="dilpesh.magneto@gmail.com">
                        <span><a style="display: none" class="resendOtp" href="javascript:void(0)">Resend OTP</a>
                    ...
                    <div class="sign-in-side">
                        ...
                        <div class="with-password add-padding-forgot">
                            ...
                            <p>Keep calm, we’ve got you covered. Just enter your email address below to reset your
                                password</p>
                            <form method="POST" id="forgotPasswordFormFromPopup" action="{{ url('/forgot-password-from-popup') }}">
                                @csrf
                                <div>
                                    <div class=" inputs-group">
                                        <input type="email" class="email" name="email" id="email">
                                ...
                                <div class="still-need-help">
                                    <a href="{{route('showSecurityQuestionCheck')}}"><span style="float:right; color:#ed4247">Still Need Help?</span></a>
                                </div>
                                <div class="login-with-psw">
                                    <span>Or Log In with<a data-bs-target="#signUpModal" data-bs-toggle="modal" data-bs-dismiss="modal">Password</a></span>
                                </div>
