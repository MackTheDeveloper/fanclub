...
<title>{{config('app.name_show')}} | Forum Comments </title>
@section('content')
   ...
   <div class="app-main__outer">
      <div class="app-main__inner">
               ...
               <div class="page-title-heading">
                  <div>
                     <div class="page-title-head center-elem">
                        ...
                        <i class="fa pe-7s-target"></i>
                        </span>
                        <span class="d-inline-block">Forum Comments</span>
                     </div>
                     <div class="page-title-subheading opacity-10">
                        <nav class="" aria-label="breadcrumb">
                           <ol class="breadcrumb">
                              ...
                              <li class="breadcrumb-item">
                                 <a href="{{route('forumsListing')}}" style="color: grey">Forums</a>
                              </li>
                              {{--
                              <li class="breadcrumb-item">
                                 <a href="javascript:void(0);" style="color: grey">forum Comments</a>
                              </li>
                              --}}
                              <li class="active breadcrumb-item" aria-current="page">
                                 <a style="color: slategray">{{$model->post_topic}} Forum's Comments</a>
                              </li>
                           </ol>
                        </nav>
                     </div>
            ...
            <div class="card-body">
               ...
               <div>
                  <form method="post" class="form-inline filter_form">
                     ...
                     {{--
                     <div class="mb-2 mr-sm-2 mb-sm-0 position-relative form-group">
                        <label for="createdBy" class="mr-sm-2">Created By</label>
                        <select name="createdBy" id="createdBy" class="multiselect-dropdown form-control" style="width: 150px;">
                     ...
                     --}}
                     <button type="button" id="filter_forum_comment" class="btn btn-primary">Search</button>
                  </form>
               </div>
            </div>
         </div>
         <div class="main-card mb-3 card">
            <div class="card-body">
               <input type="hidden" name="forum_id" id="forum_id" value="{{$model->id}}">
...
<div class="modal fade" id="forumCommentDeleteModel" tabindex="-1" role="dialog" aria-labelledby="forumCommentDeleteModelLabel" aria-hidden="true">
   <div class="modal-dialog" role="document">
      <div class="modal-content">
         <div class="modal-header">
            <h5 class="modal-title" id="forumCommentDeleteModelLabel">Confirmation</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
            </button>
         </div>
         <div class="modal-body">
            <input type="hidden" name="_token" id="token" value="{{ csrf_token() }}">
            <input type="hidden" name="forum_comment_id" id="forum_comment_id">
            <p class="mb-0" id="message_delete">Are You Sure ?</p>
         </div>
         <div class="modal-footer">
            <button type="button" class="btn btn-light" data-dismiss="modal">No</button>
            <button type="button" class="btn btn-primary" id="deleteforumComment">Yes</button>
         </div>
      </div>
   </div>
</div>
@endsection
<!-- Modal for delete template -->
<div class="modal fade" id="forumCommentViewModel" tabindex="-1" role="dialog" aria-labelledby="forumCommentViewModelLabel" aria-hidden="true">
   <div class="modal-dialog" role="document">
      <div class="modal-content">
         <div class="modal-header">
            <h5 class="modal-title" id="forumCommentViewModelLabel"></h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
            </button>
         </div>
...
@push('scripts')
<script src="{{asset('public/assets/js/forums/forumscomments.js')}}"></script>
<script>
   ...
   $(document).on('click','.forum_comment_view', function(){
       var forum_comment_id = $(this).data('id');
       var token = "{{ csrf_token() }}";
       $.ajax({
       type: "POST",
       url:"{{ route('commentView') }}",
       data:{id:forum_comment_id,_token:token},
       success: function(data){
       $('#forumCommentViewModel').on('show.bs.modal', function(e){
               $('#forumCommentViewModelLabel').text(data.name).append('&#180;s Comment');
               $('#message_delete').text(data.comment);
       });
       $('#forumCommentViewModel').modal('show');
       }
       });
