...
$(document).ready(function(){
    ...
    $('#Tdatatable').on('click', 'tbody .blog_categories_delete', function () {
        var blog_categories_id = $(this).data('id');  
        var message = "Are you sure ?";   
        console.log(message);       
        $('#blogCategoriesDeleteModel').on('show.bs.modal', function(e){
            $('#blog_categories_id').val(blog_categories_id);
            $('#message_delete').text(message);
        });
        $('#blogCategoriesDeleteModel').modal('show');              
    })

    $(document).on('click','#deleteBlogCategories', function(){
        var blog_categories_id = $('#blog_categories_id').val(); 
        $.ajax({
            url: origin + '/../delete/' + blog_categories_id,
            method: "POST",    
            data: {
                "_token": $('#token').val(),
            ...
            success: function(response)
            {
                ...
                {
                    $('#blogCategoriesDeleteModel').modal('hide')
                    DatatableInitiate();
                    toastr.clear();
    ...
    $('#Tdatatable').on('click', 'tbody .toggle-is-active-switch', function () {            
        ...
        $('#blogCategoriesIsActiveModel').on('show.bs.modal', function(e){
            $('#blog_categories_id').val(blog_categories_id);
            $('#status').val(status);
            $('#message').text(message);
        });
        $('#blogCategoriesIsActiveModel').modal('show');                                         
    });    

    
    /** Activate or deactivate blog cateogry */
    $(document).on('click','#blogCategoriesIsActive', function(){ 
        var blog_categories_id = $('#blog_categories_id').val();
        ...
        $.ajax({
            ...
            success: function(response)
            {
                if(response.status == 'true')
                {                    
                    $('#blogCategoriesIsActiveModel').modal('hide')
                    DatatableInitiate();
                    toastr.clear();
                    toastr.options.closeButton = true;
                    toastr.options.timeOut = 0;
