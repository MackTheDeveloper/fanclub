...
$(document).ready(function () {
    ...
    $(document).on('click', '#deleteProducts', function () {
        var related_products_id = $('#products_id').val();
        $.ajax({
            url: origin + '/../' + product_id + '/delete',
            method: "POST",
            data: {
                "_token": $('#token').val(),
                products_id: related_products_id,
            },
...
$(document).on('click', '.addRelatedProductsBtn', function () {
        // var products_id = $(this).data('id');
        // var message = "Are you sure ?";
        // console.log(message);
        // $('#relatedProductsModal').on('show.bs.modal', function (e) {
            // $('#products_id').val(products_id);
            // $('#message_delete').text(message);
        // });
        $('#addRelatedProductModel').modal('show');
})
$(document).on('click', '#addRelatedProducts', function () {
        
        var related_product_id = $('#related_product_id').val();
        // alert(related_product_id);
        // return false;
        $.ajax({
            url: origin + '/../' + product_id + '/store',
            method: "POST",
            data: {
                "_token": $('#token').val(),
            ...
            success: function (response) {
                    $('#addRelatedProductModel').modal('hide')
                    DatatableInitiate();
                    toastr.clear();
                    toastr.options.closeButton = true;
                    toastr.options.timeOut = 0;
    ...
    $('#Tdatatable').DataTable(
        {
            ...
            "scrollX": true,
            "ajax": {
                url: "list/"+product_id, // json datasource
                data: function (d) {
                    d.fromDate = fromDate;
                    d.toDate = toDate;
